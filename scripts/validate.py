...
def checks(root: Path=ROOT) -> dict:
    ...
    hashes=json.loads(read('reference/hardware-sha256.json'))
    for path,digest in hashes.items():
        p=root/'vendor/modu-c-firmware'/path
        need(p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()==digest,'Vendored hardware changed: '+path)
    return {'result':'PASS','layer_count':len(layers),'binding_count':sum(len(b) for _,b in layers),
            ...
            'unmodified_vendor_files':len(hashes),'firmware_compiled':False,'hardware_tested':False}
