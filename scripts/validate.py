...
def check_build_files() -> None:
    ...
    expected = [
        ...
        ("nice_nano/nrf52840/zmk", "modu_dongle", "modu_dongle"),
        ("nice_nano/nrf52840/zmk", "modu_dongle_oled", "modu_dongle_oled"),
        (BOARD, "settings_reset", "settings_reset_modu"),
        ("nice_nano/nrf52840/zmk", "settings_reset", "settings_reset_dongle"),
    ]
    actual = [(e["board"], e["shield"], e["artifact"]) for e in entries]
    if actual != expected:
        fail(f"wrong build targets: {actual}")
    if (ROOT / "zephyr/module.yml").exists():
        fail("root module changes reusable-workflow paths; matrix paths must be migrated together")
    for e in entries:
        args = shlex.split(e["cmake"])
        modules = [a for a in args if a.startswith("-DZMK_EXTRA_MODULES=")]
        if len(modules) != 1:
            fail(f"expected exactly one quoted module-list argument: {e['shield']}")
        for module in modules[0].split("=", 1)[1].split(";"):
            if not module.startswith("${GITHUB_WORKSPACE}/"):
                fail(f"unexpected module path: {module}")
            relative = module.removeprefix("${GITHUB_WORKSPACE}/")
            if not relative.startswith("modu-c-firmware/") and not (ROOT / relative / "zephyr/module.yml").exists():
                fail(f"local module missing: {relative}")
        if e["shield"] != "settings_reset":
            if "-DKEYMAP_FILE=${GITHUB_WORKSPACE}/config/modu.keymap" not in args:
                fail(f"keymap not explicit for {e['shield']}")
        if e["artifact"] in ("modu_left", "modu_right"):
            for required in ("-DCONFIG_ZMK_SPLIT=y", "-DCONFIG_ZMK_SPLIT_ROLE_CENTRAL=n", "-DCONFIG_ZMK_USB=n"):
                if required not in args:
                    fail(f"missing peripheral flag {required}")
            overlay = f"config/dongle/{e['artifact']}.overlay"
            if not (ROOT / overlay).exists() or f"-DEXTRA_DTC_OVERLAY_FILE=${{GITHUB_WORKSPACE}}/{overlay}" not in args:
                fail(f"missing overlay {overlay}")
    west = (ROOT / "config/west.yml").read_text()
    if _manifest_project(west, "zmk").get("revision") != ZMK_REVISION:
        fail("ZMK source revision changed")
    if _manifest_project(west, "modu-c-firmware").get("revision") != MODU_REVISION:
        fail("MODU source revision changed")
    wf = (ROOT / ".github/workflows/build.yml").read_text()
    for required in (f"build-user-config.yml@{ZMK_REVISION}", "local-modules/**", "fallback_binary: hex", "python3 scripts/validate.py", "python3 scripts/selftest.py", "python3 scripts/test_full_config.py", "python3 scripts/package_firmware.py", "modu-c-dongle-firmware"):
        if required not in wf:
            fail(f"workflow missing {required}")
    for f in REQUIRED_LICENSE_FILES:
        if not (ROOT / f).is_file():
            fail(f"missing license/notice: {f}")
