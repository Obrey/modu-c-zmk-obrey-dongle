...
static int pmw3610_async_init_configure(const struct device *dev) {
    ...
    if (!err) {
        err = pmw3610_set_cpi(dev, config->cpi, data->active_swap_xy, data->active_inv_x,
                              data->active_inv_y);
    }
...
static void pmw3610_async_init(struct k_work *work) {
    ...
    if (data->err) {
        ...
        if (data->async_init_step == ASYNC_INIT_STEP_COUNT) {
            ...
            LOG_INF("PMW3610 motion polling every %d ms", CONFIG_PMW3610_ALT_POLL_INTERVAL_MS);
            k_timer_start(&data->poll_timer, K_MSEC(CONFIG_PMW3610_ALT_POLL_INTERVAL_MS),
                          K_MSEC(CONFIG_PMW3610_ALT_POLL_INTERVAL_MS));
#else
...
#define TOINT16(val, bits) (((struct { int16_t value : bits; }){val}).value)
    ...
    if (data->active_rotation_cos_milli != 1000 || data->active_rotation_sin_milli != 0) {
        int32_t rotated_x = ((int32_t)x * data->active_rotation_cos_milli -
                             (int32_t)y * data->active_rotation_sin_milli) /
                            1000;
        int32_t rotated_y = ((int32_t)x * data->active_rotation_sin_milli +
                             (int32_t)y * data->active_rotation_cos_milli) /
                            1000;

        x = (int16_t)CLAMP(rotated_x, INT16_MIN, INT16_MAX);
        y = (int16_t)CLAMP(rotated_y, INT16_MIN, INT16_MAX);
        LOG_DBG("rotated x/y: %d/%d", x, y);
    }
...
static void pmw3610_poll_timer_expiry(struct k_timer *timer) {
    ...
    // timer runs in ISR context; SPI read must happen in the workqueue thread
    k_work_submit(&data->trigger_work);
}
#endif
...
static void pmw3610_init_orientation(const struct device *dev) {
    struct pixart_data *data = dev->data;
    const struct pixart_config *config = dev->config;

    data->active_rotation_cos_milli = config->rotation_cos_milli;
    data->active_rotation_sin_milli = config->rotation_sin_milli;
    data->active_swap_xy = config->swap_xy;
    data->active_inv_x = config->inv_x;
    data->active_inv_y = config->inv_y;

    if (!config->orientation_gpio.port) {
        LOG_INF("Using default rotation cos/sin: %d/%d",
                data->active_rotation_cos_milli, data->active_rotation_sin_milli);
        return;
    }

    if (!device_is_ready(config->orientation_gpio.port)) {
        LOG_WRN("Orientation GPIO device not ready; using default rotation");
        return;
    }

    int err = gpio_pin_configure_dt(&config->orientation_gpio, GPIO_INPUT);
    if (err) {
        LOG_WRN("Failed to configure orientation GPIO (%d); using default rotation", err);
        return;
    }

    int selected = gpio_pin_get_dt(&config->orientation_gpio);
    if (selected < 0) {
        LOG_WRN("Failed to read orientation GPIO (%d); using default rotation", selected);
        return;
    }

    if (selected > 0) {
        data->active_rotation_cos_milli = config->alt_rotation_cos_milli;
        data->active_rotation_sin_milli = config->alt_rotation_sin_milli;
        data->active_swap_xy = config->alt_swap_xy;
        data->active_inv_x = config->alt_inv_x;
        data->active_inv_y = config->alt_inv_y;
        LOG_INF("Orientation GPIO active; using alternate axis/rotation: swap %d inv_x %d inv_y %d cos/sin %d/%d",
                data->active_swap_xy, data->active_inv_x, data->active_inv_y,
                data->active_rotation_cos_milli, data->active_rotation_sin_milli);
    } else {
        LOG_INF("Orientation GPIO inactive; using default axis/rotation: swap %d inv_x %d inv_y %d cos/sin %d/%d",
                data->active_swap_xy, data->active_inv_x, data->active_inv_y,
                data->active_rotation_cos_milli, data->active_rotation_sin_milli);
    }
}
...
static int pmw3610_init(const struct device *dev) {
    ...
    // Select rotation matrix once at boot.
    pmw3610_init_orientation(dev);
...
#define PMW3610_DEFINE(n)                                                                          \
    ...
    static const struct pixart_config config##n = {                                                \
        ...
        .orientation_gpio = GPIO_DT_SPEC_INST_GET_OR(n, orientation_gpios, {0}),                   \
        .cpi = DT_PROP(DT_DRV_INST(n), cpi),                                                       \
        .swap_xy = DT_PROP(DT_DRV_INST(n), swap_xy),                                               \
        .inv_x = DT_PROP(DT_DRV_INST(n), invert_x),                                                \
        .inv_y = DT_PROP(DT_DRV_INST(n), invert_y),                                                \
        .rotation_cos_milli = DT_PROP(DT_DRV_INST(n), rotation_cos_milli),                         \
        .rotation_sin_milli = DT_PROP(DT_DRV_INST(n), rotation_sin_milli),                         \
        .alt_rotation_cos_milli = (int16_t)DT_PROP(DT_DRV_INST(n), alt_rotation_cos_milli),        \
        .alt_rotation_sin_milli = (int16_t)DT_PROP(DT_DRV_INST(n), alt_rotation_sin_milli),        \
        .alt_swap_xy = DT_PROP(DT_DRV_INST(n), alt_swap_xy),                                       \
        .alt_inv_x = DT_PROP(DT_DRV_INST(n), alt_invert_x),                                        \
        .alt_inv_y = DT_PROP(DT_DRV_INST(n), alt_invert_y),                                        \
