/* SPDX-License-Identifier: MIT -- battery widget logic only, NOT original UI rendering. */
...
int main(void) {
    ...
    assert(zmk_widget_dongle_battery_status_obj(&widget)!=NULL);
    assert(zmk_widget_dongle_battery_status_init(&widget, NULL)==-EALREADY);
    assert(strcmp(half_labels[0]->text,"L  --%")==0);
    ...
    assert(strcmp(half_labels[0]->text,"L  83%")==0);
    assert(strcmp(half_labels[1]->text,"R  77%")==0);
    ...
    refresh_batteries(NULL); assert(strcmp(half_labels[0]->text,"L  83%")==0);
    ...
    battery_event(1,0); refresh_batteries(NULL);
    assert(strcmp(half_labels[0]->text,"L   0%")==0);
    mock_conns[1].info.state=0;
    modu_status_conn_callbacks.disconnected(&mock_conns[1],0);
    refresh_batteries(NULL); assert(strcmp(half_labels[0]->text,"L OFF")==0);
    mock_conns[1].info.state=BT_CONN_STATE_CONNECTED;
    refresh_batteries(NULL); assert(strcmp(half_labels[0]->text,"L  --%")==0);
    battery_event(1,55); refresh_batteries(NULL);
    assert(strcmp(half_labels[0]->text,"L  55%")==0);
    battery_event(1,101); refresh_batteries(NULL);
    assert(strcmp(half_labels[0]->text,"L  --%")==0);
    /* Connecting a different peer must never re-use a previous peer's L label. */
    mock_conns[1].addr.a.val[0]=99; refresh_batteries(NULL);
    assert(strcmp(half_labels[0]->text,"L OFF")==0);
    key_event(1,1); battery_event(1,64); refresh_batteries(NULL);
    assert(strcmp(half_labels[0]->text,"L  64%")==0);
#if CONFIG_MODU_DONGLE_HAS_BATTERY
    assert(strcmp(power_label->text,"D  --%")==0);
    zmk_event_t local = {.kind=3,.local={.state_of_charge=42}};
    status_event(&local); refresh_batteries(NULL);
    assert(strcmp(power_label->text,"D  42%")==0);
    mock_usb=false; refresh_batteries(NULL); assert(strcmp(power_label->text,"D  42%")==0);
#else
    assert(strcmp(power_label->text,"D  USB")==0);
    mock_usb=false; refresh_batteries(NULL); assert(strcmp(power_label->text,"D  EXT")==0);
#endif
    unsigned n=object_count;
    for (int i=0;i<1000;i++) refresh_batteries(NULL);
    ...
    puts("Battery widget simulation passed (NOT an ARM/LVGL/Zephyr build or OLED rendering test)");
    return 0;
}
